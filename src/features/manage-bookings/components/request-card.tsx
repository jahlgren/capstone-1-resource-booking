...
import { Calendar, Check, DollarSign, MessageSquareQuote, Star, Wallet, X } from "lucide-react";
...
export default function RequestCard(
    { request }: { request: ManageBookingRequest },
) {
    const { mutate: updateStatus, isPending: isUpdating } =
        useManageBookingsMutation();

    const { data: ratings } = useResourceRatingsQuery(request.resourceId);

    const renterReview = ratings?.find((r) => r.userId === request.renterId);
    ...
    return (
        <div className="group bg-white border border-slate-100 rounded-[2.5rem] flex flex-col gap-0 hover:shadow-xl transition-all duration-500 overflow-hidden">
            ...
            {renterReview && (
                <div className="px-6 pb-6 pt-6 pt-0 border-t border-slate-100 bg-gb-green/5">
                    <div className="rounded-2xl border border-gb-green/50 bg-white/90 px-4 py-3 space-y-2">
                        ...
                        <div className="flex items-center gap-0.5" aria-hidden>
                            {Array.from({ length: 5 }, (_, i) => (
                                <Star
                                    key={i}
                                    className={
                                        i < renterReview.stars
                                            ? "size-4 fill-amber-400 text-amber-400"
                                            : "size-4 fill-slate-200 text-slate-200"
                                    }
                                    strokeWidth={i < renterReview.stars ? 0 : 1.5}
                                />
                            ))}
                            <span className="ml-2 text-sm font-black tabular-nums text-slate-800">
                                {renterReview.stars}/5
                            </span>
                        </div>
                        {renterReview.comment
                            ? (
                                <p className="text-sm text-slate-600 leading-relaxed font-medium pl-0.5">
                                    {renterReview.comment}
                                </p>
                            )
                            : null}
