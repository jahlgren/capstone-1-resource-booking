...
import {createRoesourceSchema, CreateResourceInput } from "../validation/create-resource-validator";
...
export default function useCreateResourceMutation() {
    ...
    return useMutation<Resource, Error, CreateResourceInput, { toastId: string }>({
        ...
        onSuccess: (data, variables, context) => {
         ...
         onError: (err, _, context) => {
            toast.error((
                <div>
                    <strong className="font-medium">Företagsskapandet misslyckades</strong>
                    <p>{err.message}</p>
                </div>
            ), {
                id: context?.toastId,
